...
export default function Account(props) {
    ...
    if (typeof props.setInjectedProvider == "function") {
        if (web3Modal.cachedProvider) {
            modalButtons.push(
                <Button key="logoutbutton" style={{verticalAlign: "top", marginLeft: 8, marginTop: 4}} shape={"round"}
                        size={"large"} onClick={logoutOfWeb3Modal}>logout</Button>
            )
        } else {
            modalButtons.push(
                <Button key="loginbutton" style={{verticalAlign: "top", marginLeft: 8, marginTop: 4}} shape={"round"}
                        size={"large"} type={props.minimized ? "default" : "primary"}
                        onClick={loadWeb3Modal}>connect</Button>
            )
        }
    }
    ...
    if (!props.minimized) {
        ...
        <Wallet address={props.address} provider={props.injectedProvider} ensProvider={props.mainnetProvider}
                price={props.price}/>
      </span>
        )
    }
...
Account.propTypes = {
    ...
    localProvider: PropTypes.object,
    injectedProvider: PropTypes.object,
    mainnnetProvide: PropTypes.object,
    setInjectedProvider: PropTypes.func,
